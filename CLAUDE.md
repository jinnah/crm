...
**Public form, two-way SMS, notifications and response tracking milestone complete — awaiting lead-architect review.** Phases 0–1, the core CRM milestone and the inbound-channel milestone are approved.

This milestone adds: the public `/request` form served by the same Next.js app through a same-origin BFF route (`/api/public-request`) that holds the form secret server-side, enforces an actual-byte limit and bounded per-IP throttling; owner-only communication settings (business name, form text, acknowledgment/alert toggles and templates with a fixed safe variable set, notification number, response target); a durable `outbound_messages` record written before the provider is contacted, deduplicated by idempotency-key digest and provider SID; staff-originated SMS with role checks (team members only on assigned leads), archived-lead rejection and pending-send protection; automated acknowledgment and staff alert after a stored web-form event, deduplicated by inbound activity and purpose; signed Twilio delivery-status callbacks applied idempotently; and first-response tracking (clock starts on first inbound, only human SMS or an explicit "Mark contacted outside CRM" stops it) shown on the lead detail, lead list and attention queue.

Corrections in this milestone: the 64 KB inbound limit is enforced by an ASGI middleware before FastAPI reads or parses the body (no `request._body` mutation); CRM-write retries are selective (network/timeout/429/5xx retried in a bounded loop, 4xx never); n8n upgraded 1.64.0 → 2.11.3 (`update:workflow --active` replaced by `publish:workflow --id`); channel workflows set `saveDataSuccessExecution: none` so rejected/invalid payloads leave no personal data in execution history; the n8n editor binds to loopback only.

Deferred to later phases: outbound WhatsApp/Facebook/email, Gmail/Outlook, marketing or bulk messaging, media storage, lead merging, appointment booking, payments, Sheets integration and import, production deployment automation.
