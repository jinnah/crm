...
The UI is a token-driven "calm service operations" design: every color, spacing step, radius, shadow and the focus ring are CSS variables at the top of `web/src/app/globals.css`, and screens compose the shared component set in `web/src/components/ui/` (buttons, badges, cards, page headers, dialogs, toasts, tabs, empty states, actions menus) rather than styling their own markup. Soft neutral canvas, white surfaces, deep navy navigation, teal-blue primary actions, amber for attention, red only for destruction and confirmed failure; one light theme; 8px spacing scale; icons from `lucide-react`. Tables pair with stacked record cards below 768px — never horizontal page scrolling. The business logo (owner-uploaded, server-normalized, stored in PostgreSQL, served at `/api/v1/public/logo`) appears in the shell and on public pages, with an initials wordmark as the fallback.

## Current phase

**UI redesign + business branding (corrective item 7) complete — awaiting lead-architect review.** The appointment-scheduling milestone and everything before it are approved through their respective reviews; this corrective pass restyled every screen (sidebar shell, Today dashboard, Leads workspace, time-axis calendar, sectioned Settings, Users administration, public booking/appointment pages), added the reusable component set above, and added owner-only logo management (decode-verified uploads, 1 MiB streamed limit, metadata stripping, re-encoding, loss-safe migration `c7a41d92e6b3`).
